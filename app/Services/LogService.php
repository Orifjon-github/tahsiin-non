...
class LogService
{
    ...
    public function response(string $service, string $id, int $response_code, string $additional = null, ?array $data = [], $time = 0): void
    {
        $request_code = request()->header('RequestCode');
        Log::channel($service)->info(
            "RESPONSE time: $time $request_code - $id $response_code :$additional",
            $data ?: []
        );
    }
}
